...
if (Test-Path -Path "C:\DFIR\ArtiFast") {
    ...
    New-Item -Path "C:\DFIR\_Tools\Forensic Suits\ArtiFast" -ItemType SymbolicLink -Target "C:\DFIR\ArtiFast" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\ArtiFast"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\Forensic Suits\ArtiFast" -ItemType SymbolicLink -Target "C:\DFIR\ArtiFast\ArtiFast.exe" -Force
