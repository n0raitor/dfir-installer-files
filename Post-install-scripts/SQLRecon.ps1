...
if (Test-Path -Path "C:\DFIR\SQLRecon") {
    ...
    New-Item -Path "C:\DFIR\_Tools\Offensive\SQLRecon" -ItemType SymbolicLink -Target "C:\DFIR\SQLRecon" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\SQLRecon"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\Offensive\SQLRecon" -ItemType SymbolicLink -Target "C:\DFIR\SQLRecon\SQLRecon.exe" -Force
