...
if (Test-Path -Path "C:\DFIR\WindowsTerminal") {
    ...
    New-Item -Path "C:\DFIR\_Tools\System Utilities\WindowsTerminal" -ItemType SymbolicLink -Target "C:\DFIR\WindowsTerminal" -Force
} else {
    Write-Host "Tool directory does not exist: C:\DFIR\WindowsTerminal"
}

# Create symlink
#New-Item -Path "C:\DFIR\_Tools\Recommended\WindowsTerminal" -ItemType SymbolicLink -Target "C:\DFIR\WindowsTerminal\WindowsTerminal.exe" -Force
